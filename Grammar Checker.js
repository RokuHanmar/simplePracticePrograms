...
storyWords.forEach(function calculateLength(word){
  count++;
});
console.log(count);

storyWords = storyWords.filter(function removeFiller(word){
  if (word != unnecessaryWord) {
    return word;
  }
})

storyWords = storyWords.map(function correctSpelling(word){
...
badWordIndex = storyWords.findIndex(function sanitise(word){
  if (word === badWord) {
    return word;
  }
});
...
lengthCheck = storyWords.every(function simplify(word){
  if (word.length <= 10) {
    return true;
  } else {
    return false;
...
/*
console.log(storyWords.filter(function findLongWords(word){
